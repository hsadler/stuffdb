...
def get_simple_datatype_from_mysql_datatype(mysql_datatype):
    mysql_datatype = mysql_datatype.upper()
    for key, items in datatype_map.iteritems():
        for item in items:
            if item in mysql_datatype:
                return key
    return None
...
def generate_string_insert_values(count, cardinality):
    int_values = generate_integer_insert_values(count, cardinality)
...
def insert_to_table(cur, table_name, column_insert_data, count, batch_count):
    ...
    for insert_data in column_insert_data:
        ...
        if simple_datatype == 'int':
            ...
            values = generate_string_insert_values(
                count=count,
                cardinality=cardinality
            )
        col_data = {
